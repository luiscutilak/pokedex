const pokemonListHtml = document.getElementById('pokemonList') //aqui pegamos somente a lista de pokemons da <ol> no HTML
...
pokemonListHtml.addEventListener('click', function(event) {
    ...
    if (clickedPokemon) {
        ...
        const pokemonName = pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1); //atribuindo o retorno do JSON transformado em objto e manipulando  nome, primeira letra(charAt), toUpperCase em letra maiuscula, name.slice(1), retira as duas primeiras chaves do objeto.
...
//     return `
//         <progress class="stat ${pokemon.results.stats.hp}">
//             <span class="number">#${pokemon.number}</span>
//             <span class="name">${pokemon.name}</span>

//             <progress class="stat ${pokemon.results.stats.attack}">
//             <span class="number">#${pokemon.number}</span>
//             <span class="name">${pokemon.name}</span>

//             <progress class="stat ${pokemon.results.stats.defense}">
//             <span class="number">#${pokemon.number}</span>
//             <span class="name">${pokemon.name}</span>
    
//             <div class="detail">
//                 <progress class="stats">
//                     ${pokemon.stats.map((stat) => `<progress class="type ${stat}">${stat}</progress>`).join('')}
//                 </progress>

//                 <img <button id="btn-modal" type="button" onclick="openModal(event)"></button> src="${pokemon.photo}"
//                      alt="${pokemon.name}">
//             </div>
//         </progress>
//     `
